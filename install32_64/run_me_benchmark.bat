...
set DLL_CMD=%PIN_DIR%\pin.exe -t %PINTOOL% -m "%TRACED_MODULE%" -o %OUT_PATH% -s %SETTINGS_FILE% -b "%WATCH_ARGS%" -x "%EXCLUDED_FUNC%" -l "%SYSCALLS_TABLE%" -- "%DLL_LOAD%" "%TARGET_APP%" %DLL_EXPORTS%
set EXE_CMD=%PIN_DIR%\pin.exe -t %PINTOOL% -m "%TRACED_MODULE%" -o %OUT_PATH% -s %SETTINGS_FILE% -b "%WATCH_ARGS%" -x "%EXCLUDED_FUNC%" -l "%SYSCALLS_TABLE%" -- "%TARGET_APP%" %EXE_ARGS%
